...
interface
 ...
 { С Б О Р Н Ы Е   Ф У Н К Ц И И}

  procedure bgroundAct;              // объекты окружения
  procedure bulletsAct;              // пули
  procedure enemiesAct;              // противник
  procedure playerAct;               // игрок 1
  procedure player2Act;              // игрок 2
  procedure GUI;                     // интерфейс
  procedure spawnEN;                 // создание противника
  procedure controlsKD(key0:word);   // управление
  procedure controlsKU(key0:word);   // управление

implementation

  {
      В данном юните представлены сборные функции для объектов,
    пользовательского интерфейса и управлния, облегчающие разработчику
    читабельность кода в функциях таймеров и ускоряющих процесс отладки программы.                }
...
var
  ...
  begin
    ...
    if (Key0 = ord('I')) and (not(Spawned)) then begin
        Spawned := True;
        isExist:=not(isExist);     // спавн Игрока 1
        Player.setHP(300);
    end;

    if (Key0 = ord('O')) and (not(Spawned2)) and (isExist) then begin
        Spawned2 := True;
        isExist2:=not(isExist2);   // спавн Игрока 2
        Player2.setHP(300);
    end;
