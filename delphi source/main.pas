...
begin
  ...
  plSpeed := 4;     // нормальая скорость игрока
  Score := 0;       // счетчик очков
  KOF:=1;           // коэффициент кол-ва врагов
  Pause := true;
  isExist:= false; isExist2:=false;
  Killed:=false; Killed2:=false;
  Boss := False;

  DelayFS:=0;    // задержка появления 'FAST' врагов
  DelayBB:=0;    // задержка появления ' BB ' врагов
  DelayBG:=0;    // замедление анимации фона

  // Загрузка графики
  DoubleBuffered := True; // устранение мерцания
  loadGraphics;           // загрузка текстур
...
begin
  ...
  If Key = VK_Escape then begin
    ...
    pause := not(pause);
    Spawn.Enabled:=pause;
    Update.Enabled:=pause;   // остановка таймеров
...
begin
  ...
  // Отдельное облако рисуется последним для эффекта пролета под ним !
